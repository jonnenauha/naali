...
// static
boost::shared_ptr<RexNetworking::InventorySkeleton> InventoryParser::ExtractInventoryFromXMLRPCReply(XmlRpcEpi &call)
{
    boost::shared_ptr<RexNetworking::InventorySkeleton> inventory =
        boost::shared_ptr<RexNetworking::InventorySkeleton>(new RexNetworking::InventorySkeleton);
    ...
    typedef std::pair<RexUUID, RexNetworking::InventoryFolderSkeleton> DetachedInventoryFolder;
    ...
    for(DetachedInventoryFolderList::iterator iter = folders.begin(); iter != folders.end(); ++iter)
    {
        ...
        {
            RexNetworking::InventoryFolderSkeleton *root = inventory->GetRoot();
    ...
    while(folders.size() > 0 && progress)
    {
        ...
        while(iter != folders.end())
        {
            ...
            RexNetworking::InventoryFolderSkeleton *parent = inventory->GetChildFolderById(iter->first);
            if (parent)
            {
    ...
    RexNetworking::InventoryFolderSkeleton *worldLibrary = inventory->GetChildFolderById(inventoryLibraryRootFolderID);
    if (!worldLibrary)
...
// STATIC
void InventoryParser::SetErrorFolder(RexNetworking::InventoryFolderSkeleton *root)
{
    RexNetworking::InventoryFolderSkeleton errorFolder;
