...
{
    const std::string &LLOutputStream::logname = "LLOutputStream";
    const std::string LL_MESSAGE_TEMPLATE_FILE = "./data/message_template.msg";
    extern Real LL_THROTTLE_MAX_BPS;

    LLOutputStream::LLOutputStream ()
        : messagemgr_ (new LLMessageManager (LL_MESSAGE_TEMPLATE_FILE.c_str())),
        connected_(false), 
        block_serial_num_(0)
    {
        LogInfo("LLOutputStream created and ready.");
    }

    LLOutputStream::LLOutputStream (const LLStreamParameters &params)
        : messagemgr_ (new LLMessageManager (LL_MESSAGE_TEMPLATE_FILE.c_str())),
        params_ (params),
        connected_(false), 
        block_serial_num_(0)
    ...
    bool LLOutputStream::Connect (std::string address, int port)
    {
        return connected_ = messagemgr_-> ConnectTo (address.c_str(), port);
    }

    bool LLOutputStream::Disconnect ()
    {
        messagemgr_-> Disconnect ();
        return true;
    }

    bool LLOutputStream::IsConnected () const
    {
        return connected_;
    }
            
    void LLOutputStream::SetParameters (const LLStreamParameters &params)
    {
        params_ = params;
    }
    
    void LLOutputStream::SendUseCircuitCodePacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendAgentWearablesRequestPacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendCompleteAgentMovementPacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendAgentThrottlePacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendRexStartupPacket(const std::string& state)
    {
    ...
    void LLOutputStream::SendLogoutRequestPacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendChatFromViewerPacket(const std::string &text, s32 channel)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendImprovedInstantMessagePacket(const RexUUID &target, const std::string &text)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectAddPacket(const RexTypes::Vector3 &position)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDeletePacket(const uint32_t &local_id, const bool &force)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDeletePacket(const std::vector<uint32_t> &local_id_list, const bool &force)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendAgentUpdatePacket(
            Quaternion bodyrot,
            Quaternion headrot,
            uint8_t state,
    ...
    void LLOutputStream::SendObjectSelectPacket(const unsigned int object_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectSelectPacket(std::vector<entity_id_t> object_id_list)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDeselectPacket(entity_id_t object_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDeselectPacket(std::vector<entity_id_t> object_id_list)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendMultipleObjectUpdatePacket(const std::vector<ObjectUpdateInfo>& update_info_list)
    {
        if (!connected_)
            return;
        ...
        for(size_t i = 0; i < update_info_list.size(); ++i)
        {
            ...
            // Position
            Vector3 pos (OgreToOpenSimCoordinateAxes (update_info_list[i].position_));
            memcpy(&data[offset], &pos, sizeof(Vector3));
            offset += sizeof(Vector3);

            // Scale
            Vector3 scale (OgreToOpenSimCoordinateAxes (update_info_list[i].scale_));
    ...
    void LLOutputStream::SendObjectNamePacket(const std::vector<ObjectNameInfo>& name_info_list)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectGrabPacket(entity_id_t object_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDescriptionPacket(const std::vector<ObjectDescriptionInfo>& description_info_list)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendRegionHandshakeReplyPacket(RexUUID agent_id, RexUUID session_id, uint32_t flags)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendAgentSetAppearancePacket()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendModifyLandPacket(f32 x, f32 y, u8 brush, u8 action, Real seconds, Real height)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendTextureDetail(const RexTypes::RexAssetID &new_texture_id, uint texture_index)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendTextureHeightsMessage(Real start_height, Real height_range, uint corner)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendTextureCommitMessage()
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendCreateInventoryFolderPacket(
    ...
    void LLOutputStream::SendMoveInventoryFolderPacket(
            const RexUUID &folder_id,
            const RexUUID &parent_id,
    ...
    void LLOutputStream::SendRemoveInventoryFolderPacket(
            const RexUUID &folder_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendMoveInventoryItemPacket(
    ...
    void LLOutputStream::SendCopyInventoryItemPacket(
    ...
    void LLOutputStream::SendRemoveInventoryItemPacket(const RexUUID &item_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendRemoveInventoryItemPacket(std::list<RexUUID> item_id_list)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendUpdateInventoryFolderPacket(
            const RexUUID &folder_id,
            const RexUUID &parent_id,
            const int8_t &type,
            const std::string &name)
    ...
    void LLOutputStream::SendUpdateInventoryItemPacket(
            const RexUUID &item_id,
            const RexUUID &folder_id,
    ...
    void LLOutputStream::SendFetchInventoryDescendentsPacket(
            const RexUUID &folder_id,
    ...
    void LLOutputStream::SendAcceptFriendshipPacket(const RexUUID &transaction_id, const RexUUID &folder_id)
    {
    ...
    void LLOutputStream::SendDeclineFriendshipPacket(const RexUUID &transaction_id)
    {
    ...
    void LLOutputStream::SendFormFriendshipPacket(const RexUUID &dest_id)
    {
    ...
    void LLOutputStream::SendTerminateFriendshipPacket(const RexUUID &other_id)
    {
    ...
    void LLOutputStream::SendGenericMessage(QString method, QStringList& strings)
    {
    ...
    void LLOutputStream::SendGenericMessage(const std::string& method, const StringVector& strings)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendGenericMessageBinary(
    ...
    void LLOutputStream::SendAgentPausePacket()
    {
    ...
    void LLOutputStream::SendAgentResumePacket()
    {
    ...
    void LLOutputStream::SendObjectDeRezPacket(const unsigned long ent_id, const QString &trash_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectUndoPacket(const QString &ent_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectRedoPacket(const QString &ent_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDuplicatePacket(
    ...
    void LLOutputStream::SendObjectDuplicatePacket(const unsigned long ent_id, const unsigned long flags)
    {
        SendObjectDuplicatePacket(ent_id, flags, Vector3df(0,0,0));
    }

    void LLOutputStream::SendUUIDNameRequestPacket(const RexUUID &user_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendUUIDNameRequestPacket(const std::vector<RexUUID> &user_ids)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendUUIDGroupNameRequestPacket(const RexUUID &group_id)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendUUIDGroupNameRequestPacket(const std::vector<RexUUID> &group_ids)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectLinkPacket(const std::vector<entity_id_t> &local_ids)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectLinkPacket(const QStringList& strings)
    {
    ...
    void LLOutputStream::SendObjectDelinkPacket(const std::vector<entity_id_t> &local_ids)
    {
        if (!connected_)
            return;
    ...
    void LLOutputStream::SendObjectDelinkPacket(const QStringList& strings)
    {
        std::vector<entity_id_t> vec;
    ...
    void LLOutputStream::SendLoginSuccessfullPackets()
    {
    ...
    LLOutMessage *LLOutputStream::StartMessageBuilding(const LLMsgID &message_id)
    {
        return messagemgr_->StartNewMessage(message_id);
    }

    void LLOutputStream::FinishMessageBuilding(LLOutMessage *msg)
    {
        messagemgr_->FinishMessage(msg);
    }

    void LLOutputStream::WriteFloatToBytes(float value, uint8_t* bytes, int& idx)
    {
