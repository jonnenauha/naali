...
/**
 * LLMessageException is exception type which is throwed in cases if you use LLMessage interface and a problem arises.
 * @see Exception
 */
 
class LLMessageException : public Exception
{
public:
    LLMessageException() : Exception(), strMessage_(""), type_(ET_None) {}
    virtual ~LLMessageException() throw () {} 
     ...
     */
    explicit LLMessageException(const std::string& message) : Exception(), strMessage_(message), type_(ET_None) {}

    /**
     * Override constructor. 
     * @param type Exception type enum. 
     */
    explicit LLMessageException(ExceptionType type) : Exception(), type_(type) {}
    ...
    const std::string &What() const
    {
        static const std::string exception_strings[ET_EnumCount] = {
            ...
            "Invalid nonexisting variableName input to LLInMessage::SkipToFirstVariableByName"
            };
