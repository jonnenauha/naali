...
namespace RexNetworking
{
    ...
    */
    ...
    static std::vector<uint32_t> GetAppendedAckList(uint8_t *data, size_t numBytes)
    {
        ...
        if ((data[0] & LLFlagAck) && (numBytes > 6))
        {
            if (numBytes > 6)
            {
    ...
    */
    ...
    const char *VariableTypeToStr(LLVariableType type)
    {
    ...
    LLMessageManager::LLMessageManager(const char *messageListFilename)
    :messageList(boost::shared_ptr<LLMessageList>(new LLMessageList(messageListFilename)))
    ,messageListener(0), 
    ...
    LLMessageManager::~LLMessageManager()
    {
        ClearMessagePoolMemory();            
        receivedSequenceNumbers.clear();
    }

    void LLMessageManager::DumpNetworkMessage(LLMsgID id, LLInMessage *msg)
    {
        const LLMessageInfo *info = msg->GetMessageInfo();
        if (!info)
        {
        ...
        while(msg->GetCurrentBlock() < msg->GetBlockCount())
        {
            ...
            const LLMessageBlock &block = info->blocks[curBlock];
            size_t blockInstanceCount = msg->ReadCurrentBlockInstanceCount();
            const LLMessageVariable &var = block.variables[msg->GetCurrentVariable()];

            if (curBlock != prevBlock || (curBlock == 0 && prevBlock == 0))
                std::cout << "  Block \"" << block.name << "\":" << std::endl;
            prevBlock = curBlock;
            
            LLVariableType varType = msg->CheckNextVariableType();
    ...
    const LLMessageInfo *LLMessageManager::GetMessageInfoByID(LLMsgID id) const
    {
        return messageList->GetMessageInfoByID(id);
    }

    void LLMessageManager::HandleInboundBytes(std::vector<uint8_t> &data)
    {
...
//        LLMsgID id = ExtractNetworkMessageNumber(&data[0], numBytes);
        ...
        try
        {
            LLInMessage msg(seqNum, &message[0], messageLength, (data[0] & LLFlagZeroCode) != 0);

            const LLMessageInfo *messageInfo = messageList->GetMessageInfoByID(msg.GetMessageID());
            if (!messageInfo)
            {
            ...
            // LLMessageManager handles all Acks and Pings. Those are not passed to the application.
    ...
    void LLMessageManager::ProcessMessages()
    {
        PROFILE (LLMessageManager_ProcessMessages);
        if (!connection)
            return;
        ...
        PROFILE(LLMessageManager_WhilePacketsAvailable);
    ...
    bool LLMessageManager::ConnectTo(const char *serverAddress, int port)
    {
        try
        {
    ...
    LLOutMessage *LLMessageManager::StartNewMessage(LLMsgID id)
    {
        ...
        const LLMessageInfo *info = messageList->GetMessageInfoByID(id);
        if (!info) 
            return 0;

        LLOutMessage *newMsg = 0;
    ...
    void LLMessageManager::FinishMessage(LLOutMessage *message)
    {
        assert(message);
...
#endif
        std::list<LLOutMessage*>::iterator newEnd = std::remove(usedMessagePool.begin(), usedMessagePool.end(), message);
        ...
        if (message->GetMessageInfo()->encoding == LLZeroEncoded)
        {
            size_t bodyLength = 0;
    ...
    void LLMessageManager::ClearMessagePoolMemory()
    {
        for(std::list<LLOutMessage*>::iterator iter = unusedMessagePool.begin(); iter != unusedMessagePool.end(); ++iter)
            delete *iter;

        // We're supposed to free up all of our memory, but someone's using it!
        assert(usedMessagePool.size() == 0 && "Warning! Unsafe teardown of LLMessageManager detected!");
        for(std::list<LLOutMessage*>::iterator iter = usedMessagePool.begin(); iter != usedMessagePool.end(); ++iter)
            delete *iter;
    ...
    void LLMessageManager::SendPendingACKs()
    {
        PROFILE(LLMessageManager_SendPendingACKs);
        ...
        while (pendingACKs.size() > 0)
        {
            ...
            LLOutMessage *m = StartNewMessage(RexNetMsgPacketAck);
            assert(m);
    ...
    void LLMessageManager::ProcessPacketACK(LLInMessage *msg)
    {
    ...
    void LLMessageManager::ProcessPacketACK(uint32_t id)
    {
        //std::cout << "Received ACK for packet " << id  << std::endl;
        RemoveMessageFromResendQueue(id);
    }

    void LLMessageManager::SendCompletePingCheck(uint8_t pingID)
    {
        LLOutMessage *m = StartNewMessage(RexNetMsgCompletePingCheck);
        assert(m);
        m->AddU8(pingID);
        FinishMessage(m);
    }

    /// A unary find predicate that looks for a LLOutMessage that has the given desired sequence number in a resendqueue container.
    class MsgSeqNumMatchPred
    {
    public:
        MsgSeqNumMatchPred(uint32_t seqNum_):seqNum(seqNum_) {}

        bool operator()(const std::pair<time_t, LLOutMessage*> &elem) const { return elem.second && elem.second->GetSequenceNumber() == seqNum; }
    private:
        uint32_t seqNum;
    };

    void LLMessageManager::AddMessageToResendQueue(LLOutMessage *msg)
    {
    ...
    void LLMessageManager::RemoveMessageFromResendQueue(uint32_t packetID)
    {
    ...
    void LLMessageManager::ProcessResendQueue()
    {
        PROFILE(LLMessageManager_ProcessResendQueue);
    ...
    void LLMessageManager::DebugSendHardcodedTestPacket()
    {
        const uint8_t data[] =
    ...
    void LLMessageManager::DebugSendHardcodedRandomPacket(size_t numBytes)
    {
        if (numBytes == 0)
            return;
